...
function CareerCompanies({ companies }: { companies: FutureCareers['companies'] }) {
  return (
    <div>
      ...
      <div className="border-y border-neutral-200 text-sm font-normal sm:inline-block">
        <CompanyTableHeader />
        <ol>
...
function CompanyTableHeader() {
  return (
    <div className="hidden h-10 items-center border-b border-neutral-200 sm:flex [&_p]:pl-3">
...
function CompanyTableRow({ index, name, url, year }: CompanyTableRowProps) {
  return (
    <li className="grid grid-cols-[22px,_auto,_1fr] items-center gap-x-1 px-7 py-6 odd:bg-neutral-100 sm:flex sm:h-10 sm:gap-0 sm:p-0">
      <p className={`text-sm text-neutral-400 sm:pl-5 ${TABLE_COLUMN_SIZE[0]}`}>{index}</p>
      <p
        className={`text-md font-medium sm:pl-3 sm:text-sm sm:font-normal ${TABLE_COLUMN_SIZE[1]}`}
      >
        {name}
      </p>
      <a
        className={`order-last col-span-2 col-start-2 ${
          url && 'mt-1'
        } w-fit text-xs text-link hover:underline sm:order-none sm:mt-0 sm:pl-3 ${
          TABLE_COLUMN_SIZE[2]
        }
        `}
        href={url}
      ...
      <p className={`pl-1 text-sm text-neutral-400 sm:pl-3.5 ${TABLE_COLUMN_SIZE[3]}`}>{year}</p>
    </li>
  );
}
