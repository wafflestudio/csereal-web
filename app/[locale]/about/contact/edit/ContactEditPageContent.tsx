...
export default function ContactEditPageContent({ data }: { data: WithLanguage<AboutContent> }) {
  ...
  const handleSubmit = async (content: BasicEditorContent) => {
    ...
    try {
      handleServerAction(await putContactAction(formData));
      successToast('연락처를 수정했습니다.');
    } catch (e) {
